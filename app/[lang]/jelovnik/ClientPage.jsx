...
export default function Jelovnik(props) {
        ...
        <div className="relative z-20 text-center px-4 w-full max-w-[1280px] mx-auto mt-20">
          ...
          >
            <h1 className="font-serif text-[42px] sm:text-6xl md:text-[85px] text-white leading-[1.1] mb-4">
              {lang === 'en' ? 'Our' : lang === 'ru' ? 'Наше' : 'Naš'} <span className="text-[#C22127] italic font-normal">{lang === 'en' ? 'Menu' : lang === 'ru' ? 'Меню' : 'Jelovnik'}</span>
            </h1>
          </motion.div>
          <motion.div
          ...
          >
            <p className="font-sans text-[15px] md:text-[19px] text-white/90 leading-relaxed tracking-wide font-medium">
              {t("Istražite pažljivo osmišljenu ponudu jela, pripremljenu od najkvalitetnijih sastojaka. Od laganog doručka do bogatih glavnih jela, svaki zalogaj je umetnost.")}
            </p>
          </motion.div>
        </div>
      </motion.section>
      ...
      >
        <div className="max-w-[760px] text-center flex flex-col items-center">
          ...
          <p className="font-serif text-[24px] md:text-[32px] font-normal text-white leading-[1.5] text-center italic">
            "{t("Gastronomija nije samo o ukusima, to je umetnost koja stvara uspomene.")}"
          </p>
        </div>
      </motion.section>
