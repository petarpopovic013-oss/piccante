...
export default function KartaPica(props) {
  ...
  const [activeCategory, setActiveCategory] = useState(drinksData[0].name);
  ...
  useEffect(() => {
    const handleScroll = () => {
      const categories = drinksData.map(c => c.name);
    ...
    <main className="min-h-screen bg-[#0a0a0a] overflow-x-hidden text-[#ededed]">
      ...
      >
        <motion.div style={{ y }} className="absolute inset-0 w-full h-[140%] -top-[20%] z-0">
          <Image
            src="/images/parlax.webp"
            alt="Karta Pića Hero Background"
            fill
